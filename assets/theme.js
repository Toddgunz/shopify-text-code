...
var SiblingPicker = class extends HTMLElement {
  ...
  _updateSelectedState(handle) {
    ...
    const tooltip = checkedInput?.nextElementSibling?.getAttribute("data-tooltip") || "";
    const activeLabel = this.querySelector("[data-sibling-active-label]");
    if (activeLabel) activeLabel.textContent = tooltip;
  }
  async _onColorChanged(event) {
    ...
    if (this.isQuickBuy) {
      event.stopPropagation();
      return this._onQuickBuyColorChanged(input, productUrl);
    }
  ...
  async _onQuickBuyColorChanged(input, productUrl) {
    ...
    try {
      ...
      console.error("SiblingPicker: failed to update quick-buy modal", error);
    }
  }
};
